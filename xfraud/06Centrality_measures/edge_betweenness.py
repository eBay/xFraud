...
for community_id in df_edge_weight.community_id.unique().tolist():
    ...
    if options.type_centrality == 'edge_betweenness_centrality':
        betweenness = nx.edge_betweenness_centrality(G, normalized=True)
    if options.type_centrality == 'edge_current_flow_betweenness_centrality':
        betweenness = nx.edge_betweenness_centrality(G, normalized=True)
    if options.type_centrality == 'edge_load_centrality':
        betweenness = nx.edge_load_centrality(G)
    # Add betweenness to the dataframe df_edge_weight
    for ((src, tgt), btw) in betweenness.items():
        idx = df_edge_weight.index[(df_edge_weight.source == src)                                   &(df_edge_weight.target == tgt)                                   &(df_edge_weight.community_id == community_id)]
        df_edge_weight.loc[idx, 'edge_btw'] = btw
...
df_edge_weight.to_csv('./results/df_edge_weight_imp-{}-{}.csv'.format(edge_agg, options.type_centrality)) # todo in main script
...
hitrate_df.to_csv('./results/topk-{}-{}-{}.csv'.format(100, edge_agg, options.type_centrality), index=True) # todo in main script   

ours = hitrate_df.loc[['all', 'comm0', 'comm1']]
print('Human annotation vs. edge betweenness (topk hit rate):', ours)

df_edge_weight['weight'].corr(df_edge_weight['edge_weight'])
...
pd.DataFrame.from_dict(results_pickle, orient='index',
...
pickle.dump(corr_pickle, open('corr.pkl', 'wb'))
print(pd.DataFrame.from_dict(corr_pickle, orient='index',
                       columns=['corr']))


# We compare distribution of the two columns using two-sample Kolmogorov-Smirnov test. The null hypothesis is that the two data distributions are identical.
...
pickle.dump(KS_test_stats_pickle, open('KS_test_stats.pkl', 'wb'))
